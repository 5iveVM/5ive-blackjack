...
test('fails fast when blackjack script account is not configured', async () => {
    ...
    assert.match(stderr, /Missing blackjack script account/i, 'expected missing script-account guard to trigger before chain calls');
});
test('fails fast when account overrides are placeholders', async () => {
    ...
    assert.notEqual(code, 0);
    assert.match(stderr, /Missing ACCOUNT_OVERRIDES for init_table/i, 'expected account override guard to trigger for localnet run');
});
